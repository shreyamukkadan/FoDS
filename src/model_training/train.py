...
def compare_feature_sets(X, y, full, forward, backward, estimator, cv, scoring) -> Tuple[str, List[str], pd.DataFrame]:
    ...
    if backward:
        candidates["Backward-selected features"] = list(backward)
    rows = []
...
def train_model_family(X_train, y_train, imbal_ratio=None, label="") -> Dict:
    models = {
...
def main() -> None:
    ...
    subtitle("Forward feature selection" if not RUN_BACKWARD_SELECTION else "Forward/backward feature selection")
    forward_features, forward_df = forward_selection(X_train, y_train, feature_names, selector, cv)
    backward_features, backward_df = (backward_elimination(X_train, y_train, feature_names, selector, cv) if RUN_BACKWARD_SELECTION else ([], pd.DataFrame()))
