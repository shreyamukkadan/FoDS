...
DATA_PATH = os.path.join(BASE_DIR, "..", "..", "data", "thyroid_cancer_risk_data.csv")
...
# The IQR test above is used only to DETECT and justify that outliers exist.
# For actual REMOVAL we use Z-score (|Z| > 3) because it is more sensitive
# to extreme individual values like TSH=2000 in a large dataset.
# IQR bounds are too wide at n=212k to catch these extreme values.

from scipy.stats import zscore

z_scores  = df[continuous_cols].apply(zscore)
mask_keep = (z_scores.abs() <= 3).all(axis=1)
...
def cramers_v(col1, col2):
    # Builds a contingency table and calculates Cramér's V effect size
...
# Test Country and Ethnicity against Diagnosis
for col in ["Country", "Ethnicity"]:
    v, p = cramers_v(df[col], df["Diagnosis"])
    n_unique = df[col].nunique()
    print(f"{col}:")
    print(f"  Unique values:  {n_unique}")
    print(f"  Cramér's V:     {v}  (0=no association, 1=perfect association)")
    print(f"  p-value:        {p}")
    print()
